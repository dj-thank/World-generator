...
def test_h3_client_uses_v2_task_flow(monkeypatch, tmp_path: Path) -> None:
    ...
    http = httpx.Client(base_url="https://api.minimax.io", transport=httpx.MockTransport(handler))
...
def test_h3_context_ir_replaces_only_text_item(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        minimax_h3,
        "download_file",
        lambda _url, destination, **_kwargs: destination.write_bytes(b"video") or destination,
    )
    ...
    assert posted[0]["content"][0]["text"] == "original prompt"
    assert posted[1]["content"][0]["text"] == "enhanced reconstruction prompt"
    assert posted[1]["content"][1]["role"] == "first_frame"
    http.close()
