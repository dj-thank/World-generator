...
console = Console()
_DEFAULT_GENERATE_OUTPUT = Path("outputs/world")
_DEFAULT_RECONSTRUCT_OUTPUT = Path("outputs/reconstructed")


@app.command()
def generate(
    prompt: Annotated[str, typer.Argument(help="World description in Japanese or English.")],
    output: Annotated[Path, typer.Option("--output", "-o")] = _DEFAULT_GENERATE_OUTPUT,
    name: Annotated[str, typer.Option("--name")] = "generated-world",
    anchor_image: Annotated[
        Path | None, typer.Option("--anchor-image", exists=True, dir_okay=False)
    ] = None,
    panorama_image: Annotated[
        Path | None, typer.Option("--panorama-image", exists=True, dir_okay=False)
    ] = None,
    duration: Annotated[int, typer.Option(min=4, max=15)] = 10,
    resolution: Annotated[H3Resolution, typer.Option()] = H3Resolution.P768,
    ratio: Annotated[AspectRatio, typer.Option()] = AspectRatio.WIDE,
    camera_path: Annotated[CameraPath, typer.Option()] = CameraPath.ARC_CLOCKWISE,
    orbit_degrees: Annotated[int, typer.Option(min=10, max=90)] = 45,
    camera_height: Annotated[float, typer.Option(min=0.5, max=3.0)] = 1.65,
    image_quality: Annotated[str | None, typer.Option()] = None,
    use_context_ir: Annotated[
        bool, typer.Option("--use-context-ir/--no-context-ir")
    ] = False,
    panorama_only: Annotated[bool, typer.Option("--panorama-only")] = False,
    no_panorama: Annotated[bool, typer.Option("--no-panorama")] = False,
    dry_run: Annotated[bool, typer.Option("--dry-run")] = False,
    strict_reconstruction: Annotated[bool, typer.Option("--strict-reconstruction")] = False,
    max_training_iterations: Annotated[int | None, typer.Option(min=100)] = None,
) -> None:
    ...
    if "panorama_viewer" in manifest.artifacts:
        console.print(f"Panorama viewer: {output.resolve() / 'viewer-panorama' / 'index.html'}")


@app.command()
def reconstruct(
    video: Annotated[Path, typer.Argument(exists=True, dir_okay=False)],
    output: Annotated[Path, typer.Option("--output", "-o")] = _DEFAULT_RECONSTRUCT_OUTPUT,
    max_training_iterations: Annotated[int | None, typer.Option(min=100)] = None,
) -> None:
...
def doctor() -> None:
    ...
    table.add_row("OPENAI_API_KEY", "configured" if settings.openai_api_key else "missing")
    table.add_row("MINIMAX_API_KEY", "configured" if settings.minimax_api_key else "missing")
