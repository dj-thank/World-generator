...
<body>
  <div id=\"hud\"><strong>{safe_title}</strong><span id=\"status\">Loading Gaussian Splat…</span><br>Desktop: click, then WASD. VR: use the Enter VR button and controller thumbsticks.</div>
  <div id=\"reticle\"></div>
  <script type=\"module\">
    ...
    // Nerfstudio exports can differ in orientation. Query parameters allow non-destructive adjustment.
    ...
    renderer.domElement.addEventListener('click', () => {{ if (!renderer.xr.isPresenting) controls.lock(); }});
    const keys = new Set();
