...
- **Feature reprojection**: DINO-style or self-supervised features should remain consistent under
  pose/depth warping when raw photometric loss is too strict.
- **Loop closure**: repeated viewpoints should recover the same appearance and geometry.
- **Static topology**: segmentation and correspondence tracks penalize object birth, death,
  duplication, and non-rigid deformation in static regions.
