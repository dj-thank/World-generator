...
charges or training begin. The current schema stores immutable geometry rules, navigable floor
constraints, one explicit camera trajectory, exposure/lens/focus restrictions, and reconstruction
targets.
...
Generated footage can violate the static-scene assumption. Typical failure modes are duplicate or
disappearing objects, non-rigid architecture, changing focal length or exposure, low or ambiguous
texture, motion blur, insufficient camera baseline, and inconsistent newly exposed surfaces.
...
For production, these values should be estimated from exported camera poses and stored in a
...
The default adapters are OpenAI Image API (`gpt-image-2`) and MiniMax Open Platform H3 V2 API. No
model weights are copied into this repository.
